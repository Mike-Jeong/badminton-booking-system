...
관리자는 월 멤버를 등록/수정/비활성화할 수 있다. 연 멤버와 동일하게 하드 삭제는 지원하지 않고 `isActive=false` 처리로 통일한다.
...
- 관리자가 새 월 멤버를 등록할 때, 화면에서 "기존 예약일에도 자동으로 추가하시겠습니까?" 확인을 거쳐 승인한 경우 — 등록한 연/월/요일과 일치하는, 이미 생성되어 있는 예약일 전체에 소급 적용한다(decisions.md D-22, 같은 이유로 관리자 확인을 거친다).
