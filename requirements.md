...
- 듀티 담당자 계정 등록/비밀번호 변경/비활성화 (28번 참고)
...
DutyPerson {
  id
  name             // 로그인 시 name+password 조합으로 계정을 특정하므로 유니크 제약을 둔다(28번)
  passwordHash     // 해시 저장(평문 저장 안 함). 알고리즘은 decisions.md D-38 참고
  isActive         // boolean, 기본값 true. 하드 삭제는 지원하지 않는다(28번, decisions.md D-37)
  createdAt
  updatedAt
...
- 패턴을 수정하면 다음 크론 실행부터 새 설정이 반영된다. 이미 생성된 과거 `BookingDay`는 생성 시점의 값을 그대로 유지한다(패턴과 실시간으로 동기화되지 않는다). `dutyPersonId`도 예외가 아니다 — 패턴의 듀티 담당자를 바꿔도 이미 생성된 과거 회차의 `dutyPersonId`는 소급 변경되지 않는다(28번 참고).
...
### 26.8 업로드 견고성 (decisions.md D-40)

회원 대부분이 카카오톡 채팅방에서 링크를 눌러 **카카오톡 인앱 브라우저(웹뷰)** 로 접속하는데, 이 환경에서는 사진 업로드가 간헐적으로 실패한다(카카오 데브톡 공개 보고, 갤럭시 Android 13·14). 우리 쪽에서 통제 가능한 범위를 다음과 같이 보강한다.

- **압축은 멈추지 않는다**: `compressImageFile`은 이미지 로딩 8초, 전체 10초의 상한을 가진다. 초과하면 예외를 던지지 않고 원본 파일을 그대로 반환해 업로드 시도를 이어간다(26.5의 폴백 철학 유지). 업로드 버튼이 "업로드 중..." 상태로 영구히 멈추는 일이 없어야 한다. 공개 셀프 업로드와 관리자 대리 업로드가 같은 함수를 공유하므로 양쪽에 동일하게 적용된다.
- **전송 전 클라이언트 사전검증**: 압축 결과(폴백된 원본 포함)가 허용 MIME(`image/jpeg`/`image/png`/`image/webp`)이 아니거나 2MB를 넘으면 서버로 보내지 않고 원인을 알 수 있는 안내를 보여준다(형식 오류 / 용량 초과). 서버 검증(26.5)은 그대로 유지되며 최종 방어선이다.
- **비-JSON 응답 처리**: 서버가 JSON이 아닌 응답(예: 요청 본문 상한 초과 시 플랫폼이 돌려주는 HTML 413)을 주더라도 업로드 실패 문구를 보여준다 — 네트워크 오류로 오인 표시하지 않는다. 실제 네트워크 오류 문구에는 다른 브라우저로 열어보라는 힌트를 포함한다.

### 26.9 공개 화면 인앱 브라우저 안내 배너 (decisions.md D-40)

모든 공개 화면(`app/(public)/**`) 상단에, 카카오톡 인앱 브라우저로 접속한 경우에만 안내 배너를 노출한다.

- 감지는 클라이언트 마운트 후 `navigator.userAgent`로만 수행한다(서버 렌더 결과와 어긋나지 않도록 초기 렌더는 비노출).
- 내용: 인앱에서는 사진 업로드가 실패할 수 있다는 안내 + "외부 브라우저로 열기" 버튼 + 닫기 버튼.
- **자동 리다이렉트는 하지 않는다.** 외부 브라우저 이동은 사용자가 버튼을 눌렀을 때만 실행한다.
- 닫으면 해당 화면 상태에서만 사라진다(별도 저장소에 기억하지 않는다).
- 문구는 ko/en 모두 제공한다(공개 화면이므로 22번 다국어 규칙 적용).
...
관리자 화면과 "게임 듀티" 화면을 분리한다. 예약일에 배정된 듀티 담당자가 관리자 비밀번호 없이 스스로 로그인해, 자신이 담당하는 예약일만 조회할 수 있는 별도 화면(`/duty/**`)을 신설한다. 지금까지 `BookingDay.dutyPerson`/`ClubDayPattern.dutyPerson`은 예약일에 붙는 자유 텍스트 이름일 뿐 로그인 계정 개념이 전혀 없었다. 이번 기능으로 듀티 담당자가 계정을 갖게 되지만, 관리자 계정(2번, 환경변수 단일 비밀번호 체제)과는 완전히 별개의 인증 체계이며 서로 영향을 주지 않는다.
...
- 완전히 별도의 로그인 화면 `/duty/login`을 둔다. 관리자 세션(`ADMIN_SESSION_COOKIE_NAME`)과는 다른 별도 쿠키를 사용하며, 관리자로 로그인한 상태와 듀티 계정으로 로그인한 상태는 서로 영향을 주지 않는다(같은 브라우저에서 동시에 둘 다 로그인해 있을 수 있다).
